...
function asset(base: string, req: express.Request, res: express.Response) { try { res.sendFile(assertInsideBase(path.join(base, (req.params as any)[0] || ''), base)); } catch (e:any) { res.status(404).json({ ok:false, error:e?.message || 'Asset not found' }); } }
