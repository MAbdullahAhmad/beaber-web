...
function apiBase() {
  ...
  try { return new URL(configured, window.location.origin).pathname.replace(/\/$/, '') || '/api'; }
  catch { return '/api'; }
}
...
const App = {
  setup() {
    ...
    return { notes, selectedId, query, filtered, selected, status, loadError, addNote, delNote, touch, logout };
  },
  template: `<div class="shell"><aside><header><h1>Beaver Notes Web</h1><button @click="logout">Logout</button></header><input class="search" v-model="query" placeholder="Search notes"><button class="new" @click="addNote">+ New note</button><p v-if="loadError" class="error">{{loadError}}</p><nav><button v-for="n in filtered" :key="n.id" :class="{active:n.id===selectedId}" @click="selectedId=n.id"><strong>{{n.title || 'Untitled'}}</strong><span>{{new Date(n.updatedAt).toLocaleString()}}</span></button></nav></aside><main v-if="selected"><input class="title" v-model="selected.title" @input="touch"><textarea v-model="selected.body" @input="touch" placeholder="Write your note..."></textarea><footer><span>{{status}}</span><button class="danger" @click="delNote">Delete</button></footer></main><main v-else class="empty"><h2>No notes yet</h2><p v-if="loadError">{{loadError}}</p><button @click="addNote">Create your first note</button></main></div>`
};
